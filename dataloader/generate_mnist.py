...
def generate_moving_mnist(training, shape=(64, 64), num_frames=50, num_images=10000, original_size=28, nums_per_image=3):
    '''
    ...
    foreground = np.empty((num_frames, num_images, width, height), dtype=np.uint8)

    for img_idx in range(num_images):
        # Randomly generate direction, speed and velocity for both images
        if img_idx % 100 == 0:
            print('seq: {}'.format(img_idx))
        ...
        for frame_idx in range(num_frames):
            ...
            foreground[frame_idx, img_idx] = (canvas[0] * 255).clip(0, 255).astype(np.uint8)

    background = generate_background(foreground.shape)
    dataset = (background + foreground).clip(0, 255)
    # background_updated = dataset - foreground
    # for i in range(background_updated.shape[0]):
    #     seq = background[i].reshape([background_updated.shape[1], -1])
    #     print(i, np.linalg.matrix_rank(seq))
    return dataset, foreground, background

def generate_background(shape, rank=5):
    '''
    Generate a low-rank matrix for background
    :param shape: shape of the moving mnist sequence: [num_frames, num_sequences, width, height]
    :param rank: rank of the background matrix, rank << num_frames (in the original moving mnist sequence, this rank = 1 -> static background)
    '''
    num_frames, num_seq, w, h = shape
    background = np.random.rand(num_seq * num_frames, rank).dot(np.random.rand(rank, w * h))
    return background.reshape([num_frames, num_seq, w, h])/np.max(background) * 255

    # background = np.empty([num_frames, num_seq, w, h])
    # for i in range(num_seq):
    #     seq = np.random.rand(num_frames, rank).dot(np.random.rand(rank, w * h))
    #     background[:, i, ...] = seq.reshape([-1, w, h])/np.max(seq)
    # return background * 255

def main(training, dest, filetype='npy', frame_size=64, num_frames=20, num_images=10000, original_size=28,
         nums_per_image=3):
    dat, foreground, background = generate_moving_mnist(training, shape=(frame_size, frame_size), num_frames=num_frames, num_images=num_images, \
                                original_size=original_size, nums_per_image=nums_per_image)
    n = num_images * num_frames
    if not os.path.exists(dest):
        os.makedirs(dest)
    if filetype == 'npy':
        np.save(os.path.join(dest, 'seq_r5.npy'), dat)
        np.save(os.path.join(dest, 'foreground_r5.npy'), foreground)
        np.save(os.path.join(dest, 'background_r5.npy'), background)
    elif filetype == 'jpg':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dest', type=str, dest='dest', default='../moving_mnist_fb/')
    ...
    parser.add_argument('--num_images', type=int, dest='num_images', default=10000)  # number of sequences to generate
